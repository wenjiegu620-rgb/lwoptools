...
MYSQL = dict(
    host=os.environ.get("MYSQL_HOST", "nlb-q47d4uw6iwr03nqaxh.cn-shanghai.nlb.aliyuncsslb.com"), port=int(os.environ.get("MYSQL_PORT", 3306)),
    user=os.environ.get("MYSQL_USER", "wenjie.gu"), password=os.environ["MYSQL_PASSWORD"],
...
MYSQL_NEW = dict(
    host=os.environ.get("MYSQL_NEW_HOST", "nlb-q47d4uw6iwr03nqaxh.cn-shanghai.nlb.aliyuncsslb.com"), port=int(os.environ.get("MYSQL_NEW_PORT", 3306)),
    user=os.environ.get("MYSQL_NEW_USER", "wenjie.gu"), password=os.environ["MYSQL_NEW_PASSWORD"],
...
def _query_new_db_daily(date_str, group=None):
    """从新库查当日采集/质检数据，格式与旧库一致。
    新库表结构：human_case（单数），human_case_node（单数），human_case_tag
    时长：LEFT JOIN human_case_tag pt ON type='produce_tags'，JSON_EXTRACT(pt.value,'$.data_info.duration')
    供应商：auth.users(name=producer) -> auth.user_group -> auth.groups(biz_type=produce)
    """
    conn = mysql_new()
    ...
    try:
        with conn.cursor() as cur:
            # 供应商过滤子句（走 auth 库）
            group_clause = "AND ag.name = %s" if group else ""
            args_produce = [_d_start, _d_end] + ([group] if group else [])
            cur.execute(f"""
                SELECT
                    ...
                    IFNULL(CAST(JSON_EXTRACT(pt.value, '$.data_info.duration') AS DECIMAL(15,4)), 0) AS vsec,
            ...
            # 当日质检数据（用 LEFT JOIN 替代 NOT EXISTS，避免全表扫描）
            args_qc = [_d_start, _d_end, _d_start, _d_end] + ([group] if group else [])
            cur.execute(f"""
                SELECT
                    hc.producer AS producer,
                    COUNT(*) AS qc_total_cases,
                    SUM(CASE WHEN q.passed = 1 THEN 1 ELSE 0 END) AS qc_passed_cases,
                    SUM(CASE WHEN q.passed = 1 THEN IFNULL(CAST(JSON_EXTRACT(pt.value,'$.data_info.duration') AS DECIMAL(15,4)),0) ELSE 0 END) / 3600.0 AS qc_h,
                ...
                JOIN (
                    ...
                    SELECT hcn.human_case_id, CASE WHEN hcn.node_status = 3 THEN 1 ELSE 0 END AS passed
                    FROM human_case_node hcn
                    LEFT JOIN human_case_node s ON s.human_case_id = hcn.human_case_id AND s.node_name = 'human_case_sampling'
            ...
            # 采集人效：今天采集的 case 里，已质检通过的时长
            args_cqc = [_d_start, _d_end, _d_start, _d_end] + ([group] if group else [])
            cur.execute(f"""
                SELECT
                    hc.producer AS producer,
                    SUM(CASE
            ...
            # 腕部相机项目 UUID 集合（新库通过 project_tag_project 查询）
            cur.execute("""
            ...
            """)
            new_wrist_uuids = set(r["uuid"] for r in cur.fetchall())

            # 原始人效：human_case_upload 节点当日 duration
            args_upload = [_d_start, _d_end] + ([group] if group else [])
            cur.execute(f"""
                SELECT
                    hc.producer AS producer,
                    hc.project_uuid AS project_id,
                    SUM(IFNULL(CAST(JSON_EXTRACT(pt.value,'$.data_info.duration') AS DECIMAL(15,4)),0)) / 3600.0 AS upload_h
                FROM human_case hc
                JOIN human_case_node hcn ON hcn.human_case_id = hc.id
                LEFT JOIN human_case_tag pt ON pt.human_case_id = hc.id AND pt.type = 'produce_tags'
                LEFT JOIN auth.users au ON au.name = hc.producer
            ...
            """, args_upload)
            upload_h_map = {}
            new_wrist_upload_map = {}
            new_pt_upload_map = {}
            for r in cur.fetchall():
                p = r["producer"]; h = float(r["upload_h"] or 0); pid = r["project_id"]
                upload_h_map[p] = upload_h_map.get(p, 0) + h
                if pid in new_wrist_uuids:
                    new_wrist_upload_map[p] = new_wrist_upload_map.get(p, 0) + h
                else:
                    new_pt_upload_map[p] = new_pt_upload_map.get(p, 0) + h

        return produce_rows, qc_map, collect_qc_map, collect_inspected_map, upload_h_map, new_wrist_uuids, new_wrist_upload_map, new_pt_upload_map
...
def _query_new_db_cumul(group=None):
    """从新库查累计数据（2026-01-01起，足够判断老人/新人）
    新库表：human_case（单数），human_case_node（单数），human_case_tag
    时长：JSON_EXTRACT(pt.value,'$.data_info.duration')
    供应商：auth.users -> auth.user_group -> auth.groups(biz_type=produce)
    """
    CUMUL_SINCE = "2026-01-01 00:00:00"
    conn = mysql_new()
    try:
        with conn.cursor() as cur:
            group_clause = "AND ag.name = %s" if group else ""
            args = [CUMUL_SINCE] + ([group] if group else [])
            cur.execute(f"""
                SELECT
                    hc.producer AS producer,
                    SUM(IFNULL(CAST(JSON_EXTRACT(pt.value,'$.data_info.duration') AS DECIMAL(15,4)),0)) / 3600.0 AS total_collect_h,
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(f"""
                SELECT
                    hc.producer AS producer,
                    SUM(CASE WHEN q.passed = 1 THEN IFNULL(CAST(JSON_EXTRACT(pt.value,'$.data_info.duration') AS DECIMAL(15,4)),0) ELSE 0 END) / 3600.0 AS week_qc_h
                FROM human_case hc
                LEFT JOIN human_case_tag pt ON pt.human_case_id = hc.id AND pt.type = 'produce_tags'
                JOIN (
                    SELECT hcn.human_case_id, CASE WHEN hcn.node_status = 3 THEN 1 ELSE 0 END AS passed
                ...
                WHERE hc.producer IN ({ph})
                GROUP BY hc.producer
            """, [_w_start, _w_end, _w_start, _w_end] + producers)
...
def vendor_collectors():
    ...
    try:
        with conn.cursor() as cur:
            if cached_daily:
                if len(cached_daily) == 6:
                    produce_rows, qc_map, collect_qc_map, collect_inspected_map, upload_h_map, week_sampling_map = cached_daily
                elif len(cached_daily) == 5:
                ...
                if produce_rows:
                    ...
                    collect_qc_map        = {r["producer"]: float(r["collect_qc_h"] or 0) for r in _cqc_rows}
                    collect_inspected_map = {r["producer"]: float(r["collect_inspected_h"] or 0) for r in _cqc_rows}
                    upload_h_map = {}  # 旧库不查 upload，由新库补充
                else:
                    qc_map = {}
                    collect_qc_map = {}
                    collect_inspected_map = {}
                ...
                # 本周抽检数据（仅 human_case_sampling 节点，周一到查询日期）
                _d = date.fromisoformat(date_str)
                week_start = str(_d - timedelta(days=_d.weekday()))
                cur.execute("""
                    SELECT hc.producer,
                        COUNT(*) AS week_sampling_total,
                        SUM(CASE WHEN hcn.node_status = 3 THEN 1 ELSE 0 END) AS week_sampling_passed,
                        SUM(IFNULL(hc.video_seconds, 0)) / 3600.0 AS week_sampling_h
                    FROM human_cases hc
                    JOIN human_case_nodes hcn ON hcn.human_case_id = hc.id
                    WHERE hcn.node_name = 'human_case_sampling'
                      ...
                      AND hc.produced_by_group = %s
                    GROUP BY hc.producer
                """, [week_start, date_str, group])
                week_sampling_map = {r["producer"]: {
                    "week_sampling_h": float(r["week_sampling_h"] or 0),
                    "week_sampling_total": int(r["week_sampling_total"] or 0),
                    "week_sampling_passed": int(r["week_sampling_passed"] or 0),
                } for r in cur.fetchall()}

                _cache_set(daily_cache_key, (produce_rows, qc_map, collect_qc_map, collect_inspected_map, upload_h_map, week_sampling_map), 300)
    ...
    for p, segs in sessions_by_p.items():
        ...
        ws = week_sampling_map.get(p, {})
        ws_total = int(ws.get("week_sampling_total") or 0)
        ws_passed = int(ws.get("week_sampling_passed") or 0)
        ws_h = float(ws.get("week_sampling_h") or 0)
        persons.append({
            "producer":           p,
            ...
            # 本周抽检
            "week_sampling_h":     round(ws_h, 2),
            "week_sampling_total": ws_total,
            "week_sampling_rate":  round(ws_passed / ws_total * 100, 1) if ws_total > 0 else 0,
            # 在线
    ...
    try:
        with conn_new.cursor() as cur2:
            ...
            if new_collect_rows:
                ...
                cur2.execute(f"SELECT uuid, name FROM project WHERE uuid IN ({nph2}) AND deleted_at IS NULL", new_pids2)
                for r in cur2.fetchall():
                    if r["uuid"] in new_collect_rows:
                        new_collect_rows[r["uuid"]]["project_name"] = r["name"]
    finally:
        conn_new.close()
