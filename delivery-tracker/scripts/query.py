...
DB_CONFIG = {
    ...
    "user":            os.environ.get("DELIVERY_DB_USER", "wenjie"),
...
NEW_DB_CONFIG = {
    ...
    "user": os.environ.get("DELIVERY_NEW_DB_USER", "wenjie"),
    ...
    if task_cap_hours and task_cap_hours > 0:
        picked_sql = f"""
            SELECT hc.project_id, hc.task_name,
                   COALESCE(hc.delivery_video_seconds, hc.video_seconds, 0) AS pack_seconds,
                   {ENV_FIELDS}
            FROM human_cases hc
            WHERE hc.project_id IN ({ph})
          AND hc.deleted_at IS NULL
...
def run_project_newdb(project_config, scene_mapping, project_entries):
            ...
            for item in project_entries:
                ...
                cur.execute(
                    ...
                    SELECT
                        ...
                        COALESCE(
                            CAST(JSON_EXTRACT(prod.value, '$.data_info.delivery_video_seconds') AS DECIMAL(18, 6)),
                            0
    ...
    for case in all_cases:
        ...
        packed_hours = (
            float(case.get("delivery_video_seconds") or 0.0) or float(case.get("video_seconds") or 0.0)
        ) / 3600.0
    ...
    qc_pass_k, qc_pass_u = aggregate(qc_pass_rows, scene_mapping)
    sem_ing_k, sem_ing_u = aggregate(sem_rows, scene_mapping)
    sem_done_k, sem_done_u = aggregate(sem_done_rows, scene_mapping)
    pose_ing_k, pose_ing_u = aggregate(pose_rows, scene_mapping)
    pose_done_k, pose_done_u = aggregate(pose_done_rows, scene_mapping)
    lab_ing_k, lab_ing_u = aggregate(lab_rows, scene_mapping)
    lab_done_k, lab_done_u = aggregate(lab_done_rows, scene_mapping)
    packed_k, packed_u = aggregate(
        _apply_task_cap_rows(packed_rows, packed_task_cap_hours),
        scene_mapping,
    )
    qc_known, qc_unknown = aggregate_qc_compat(qc_rows, scene_mapping)
